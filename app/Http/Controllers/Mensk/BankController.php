...
class BankController extends Controller
{
    ...
    public function deposit(Request $request) 
    {
	    ...
	    $validator = Validator::make($request->all(), [
	    	'coins' 	=> "integer|min:0|max:{$this->resources->coins}|max:{$this->bankRecord->depositMax('coins')}",
            'bronze' 	=> "integer|min:0|max:{$this->resources->bronze}|max:{$this->bankRecord->depositMax('bronze')}",
			'silver' 	=> "integer|min:0|max:{$this->resources->silver}|max:{$this->bankRecord->depositMax('silver')}",
			'gold' 		=> "integer|min:0|max:{$this->resources->gold}|max:{$this->bankRecord->depositMax('gold')}",
			'platinum' 	=> "integer|min:0|max:{$this->resources->platinum}|max:{$this->bankRecord->depositMax('platinum')}",
    	],
    	[
		...
		if ($validator->fails()) {
	        ...
	        return redirect('mensk/bank')->withInput();
	    }
